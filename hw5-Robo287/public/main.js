...
function selectupdbtn(id) {
    ...
    if (enrolled == "Enrolled") {
        document.querySelector("#input-enrolled").checked = true;
    } else {
        document.querySelector("#input-enrolled").checked = false;
    }
...
function update() {
    ...
    var enrolled = document.querySelector('#input-enrolled').checked;
    fetch('/students', {
        method: 'put',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ _id: rid, fname: fname, lname: lname, gpa: gpa, enrolled: enrolled.toString() })
    })
        .then(res => { if (res.ok) return res.json(); })
